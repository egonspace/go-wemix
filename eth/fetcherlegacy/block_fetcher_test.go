...
var (
	...
	genesis      = testGspec.MustCommit(testdb, triedb.NewDatabase(testdb, triedb.HashDefaults))
	unknownBlock = types.NewBlock(&types.Header{GasLimit: params.GenesisGasLimit, BaseFee: big.NewInt(params.InitialBaseFee)}, nil, nil, nil)
)
...
func (f *fetcherTester) makeHeaderFetcher(peer string, blocks map[common.Hash]*types.Block, drift time.Duration) headerRequesterFn {
	...
	return func(hash common.Hash, sink chan *mir.Response) (*mir.Request, error) {
		...
		res := &mir.Response{
			Req:  req,
			Res:  (*mir.BlockHeadersPacket)(&headers),
			Time: drift,
...
func (f *fetcherTester) makeBodyFetcher(peer string, blocks map[common.Hash]*types.Block, drift time.Duration) bodyRequesterFn {
	...
	return func(hashes []common.Hash, sink chan *mir.Response) (*mir.Request, error) {
		...
		// Return on a new thread
		bodies := make([]*mir.BlockBody, len(transactions))
		for i, txs := range transactions {
			bodies[i] = &mir.BlockBody{
				Transactions: txs,
				Uncles:       uncles[i],
			}
		}
		...
		res := &mir.Response{
			Req:  req,
			Res:  (*mir.BlockBodiesPacket)(&bodies),
			Time: drift,
			Done: make(chan error, 1), // Ignore the returned status
		}
		go func() {
...
func testConcurrentAnnouncements(t *testing.T, light bool) {
	...
	counter := uint32(0)
	firstHeaderWrapper := func(hash common.Hash, sink chan *mir.Response) (*mir.Request, error) {
		atomic.AddUint32(&counter, 1)
		return firstHeaderFetcher(hash, sink)
	}
	secondHeaderWrapper := func(hash common.Hash, sink chan *mir.Response) (*mir.Request, error) {
		atomic.AddUint32(&counter, 1)
...
func testPendingDeduplication(t *testing.T, light bool) {
	...
	counter := uint32(0)
	headerWrapper := func(hash common.Hash, sink chan *mir.Response) (*mir.Request, error) {
		atomic.AddUint32(&counter, 1)

		// Simulate a long running fetch
		resink := make(chan *mir.Response)
