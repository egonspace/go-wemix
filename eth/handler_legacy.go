...
import (
	...
	"github.com/ethereum/go-ethereum/eth/downloaderlegacy"
	...
	"github.com/ethereum/go-ethereum/eth/fetcherlegacy"
...
type handlerLegacy struct {
	...
	downloader   *downloaderlegacy.Downloader
	blockFetcher *fetcherlegacy.BlockFetcher
	txFetcher    *fetcherlegacy.TxFetcher
	peers        *legacyPeerSet
...
func newHandlerLegacy(config *handlerConfig) (*handlerLegacy, error) {
	...
	h.downloader = downloaderlegacy.New(h.checkpointNumber, config.Database, h.eventMux, h.chain, h.removePeer, success)
	...
	h.blockFetcher = fetcherlegacy.NewBlockFetcher(false, nil, h.chain.GetBlockByHash, validator, h.BroadcastBlock, heighter, nil, inserter, h.removePeer)
	h.txFetcher = fetcherlegacy.NewTxFetcher(h.txpool.Has, addTxs, fetchTx)
	return h, nil
}

// protoTracker tracks the number of active protocol handlers.
func (h *handlerLegacy) protoTracker() {
	defer h.wg.Done()
	var active int
	for {
		select {
...
// quitting.
func (h *handlerLegacy) incHandlers() bool {
	select {
...
func (h *handlerLegacy) decHandlers() {
	h.handlerDoneCh <- struct{}{}
}

// runEthPeer registers an eth peer into the joint eth/snap peerset, adds it to
// various subsystems and starts handling messages.
func (h *handlerLegacy) runMirPeer(peer *mir.Peer, handler mir.Handler) error {
	if !h.incHandlers() {
...
func (h *handlerLegacy) runSnapExtension(peer *snap.Peer, handler snap.Handler) error {
	if !h.incHandlers() {
		return p2p.DiscQuitting
	}
	defer h.decHandlers()
...
func (h *handlerLegacy) removePeer(id string) {
...
func (h *handlerLegacy) unregisterPeer(id string) {
...
func (h *handlerLegacy) Start(maxPeers int) {
	h.maxPeers = maxPeers
...
func (h *handlerLegacy) BroadcastBlock(block *types.Block, propagate bool) {
...
func (h *handlerLegacy) BroadcastTransactions(txs types.Transactions) {
	var (
...
func (h *handlerLegacy) txBroadcastLoop() {
	defer h.wg.Done()
	for {
		select {
		case event := <-h.txsCh:
...
// sync is finished.
func (h *handlerLegacy) enableSyncedFeatures() {
...
func (h *handlerLegacy) syncTransactions(p *eth.Peer) {
