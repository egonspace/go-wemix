...
type Peer interface {
	...
	RequestHeadersByHash(common.Hash, int, int, bool, chan *mir.Response) (*mir.Request, error)
	RequestHeadersByNumber(uint64, int, int, bool, chan *mir.Response) (*mir.Request, error)
	RequestBodies([]common.Hash, chan *mir.Response) (*mir.Request, error)
	RequestReceipts([]common.Hash, chan *mir.Response) (*mir.Request, error)
}
...
func (p *peerConnection) UpdateHeaderRate(delivered int, elapsed time.Duration) {
	p.rates.Update(mir.BlockHeadersMsg, elapsed, delivered)
}
...
func (p *peerConnection) UpdateBodyRate(delivered int, elapsed time.Duration) {
	p.rates.Update(mir.BlockBodiesMsg, elapsed, delivered)
}
...
func (p *peerConnection) UpdateReceiptRate(delivered int, elapsed time.Duration) {
	p.rates.Update(mir.ReceiptsMsg, elapsed, delivered)
}

// HeaderCapacity retrieves the peer's header download allowance based on its
// previously discovered throughput.
func (p *peerConnection) HeaderCapacity(targetRTT time.Duration) int {
	cap := p.rates.Capacity(mir.BlockHeadersMsg, targetRTT)
...
func (p *peerConnection) BodyCapacity(targetRTT time.Duration) int {
	cap := p.rates.Capacity(mir.BlockBodiesMsg, targetRTT)
...
func (p *peerConnection) ReceiptCapacity(targetRTT time.Duration) int {
	cap := p.rates.Capacity(mir.ReceiptsMsg, targetRTT)
