...
type typedQueue interface {
	...
	request(peer *peerConnection, req *fetchRequest, resCh chan *mir.Response) (*mir.Request, error)
	...
	deliver(peer *peerConnection, packet *mir.Response) (int, error)
}
...
func (d *Downloader) concurrentFetch(queue typedQueue, beaconMode bool) error {
	...
	responses := make(chan *mir.Response)

	// Track the currently active requests and their timeout order
	pending := make(map[string]*mir.Request)
	defer func() {
	...
	ordering := make(map[*mir.Request]int)
	timeouts := prque.New[int64, *mir.Request](func(data *mir.Request, index int) {
		ordering[data] = index
	})
	...
	stales := make(map[string]*mir.Request)
	defer func() {
		// Abort all requests on sync cycle cancellation. The requests may still
		// be fulfilled by the remote side, but the dispatcher will not wait to
		// deliver them since nobody's going to be listening.
